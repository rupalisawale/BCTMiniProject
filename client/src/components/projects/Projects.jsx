...
const Projects = ({state}) => {
    ...
    const donateEth=async(event)=>{
        ...
        const transaction = await contract.methods.donate().send({from:accounts[0],value:weiValue,gas:1800000});
        ...
        <section className="project-section">
            ...
            <div className="card-wrapper">
                 {projects!=="" && projects.map((project)=>{
                    const githubLink =`https://github.com/rupalisawale/${project.githubLink}`
                    return ( <a href= {githubLink} className="project-card" target='_blank' rel="noopener noreferrer" >
                    <div className="card-img">
                        <img src={`https://rose-abstract-mockingbird-681.mypinata.cloud/ipfs/${project.image}`} alt="" /></div>
 ...
 <Modal size='md' isOpen={modal} toggle={() => setModal(!modal)}>
                    ...
                    {/*  =========popup bootstrap end==========  */}
                    <p className='donate' onClick={() => setModal(true)}>Liked the Projects's ? Consider donating Eth's <FaDonate className='icon' /></p>
        </section>
    )
}
